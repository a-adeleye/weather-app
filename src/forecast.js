...
export async function dayTwo() {
    const url = `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&appid=86798aad821bb8c4ee2887a3873485ff&units=metric`;
  
    const response = await fetch(url);
    const data = await response.json();
  
    const icon = await data.daily[1].weather[0].icon;
    const tempFrom = await data.daily[1].temp.morn;
    const tempTo = await data.daily[1].temp.night;
    const weather = await data.daily[1].weather[0].description;
    const wind = await data.daily[1].wind_speed;
  ...
  export async function dayThree() {
    ...
    const tempFrom = await data.daily[2].temp.morn;
    const tempTo = await data.daily[2].temp.night;
    const weather = await data.daily[2].weather[0].description;
    const wind = await data.daily[2].wind_speed;
  ...
  export async function dayFour() {
    ...
    const tempFrom = await data.daily[3].temp.min;
    const tempTo = await data.daily[3].temp.max;
    const weather = await data.daily[3].weather[0].description;
    const wind = await data.daily[3].wind_speed;
